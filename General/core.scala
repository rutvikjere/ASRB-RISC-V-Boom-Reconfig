  ...
  with HasBoomFrontendParameters // TODO: Don't add this trait
{
  ...
  val validBranchesCount = brinfos.map(_.valid).reduce(_ || _)// Register to count the number of branches.
  val mispredictedBranchesCount = brinfos.map(_.mispredict).reduce(_ || _) // Register to count the number of mispredicts.
  ...
  val perfEvents = new freechips.rocketchip.rocket.EventSets(Seq(
    new freechips.rocketchip.rocket.EventSet((mask, hits) => (mask & hits).orR, Seq(
      ...
      ("load",       () => io.lsu.perf.ldCount),
      ("store",       () => io.lsu.perf.strCount),
      ("nop",       () => false.B),
      ("nop",       () => false.B),
      ("nop",       () => false.B),
      ("branch",    () => validBranchesCount))),

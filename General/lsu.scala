...
class LSUDMemIO(implicit p: Parameters, edge: TLEdgeOut) extends BoomBundle()(p)
{
  ...
  val perf = Input(new Bundle {
    val acquire = Bool()
    val release = Bool()
    val ldCount = Bool()   // Load count as input into the perf bundle
    val strCount = Bool()  // Store count as input to the perf bundle
  })
...
class LSUCoreIO(implicit p: Parameters) extends BoomBundle()(p)
{
  ...
  val perf        = Output(new Bundle {
    ...
    val release = Bool()
    val tlbMiss = Bool()
    val ldCount = Bool()  // load count in the output of the perf bundle
    val strCount = Bool() // str count in the output of the perf bundle
  })
}
  ...
  with rocket.HasL1HellaCacheParameters
{
  ...
  val ldCount = RegInit(0.U(32.W)) 
  val strCount = RegInit(0.U(32.W))
  ...
  io.core.perf.ldCount := ldCount
  io.core.perf.strCount := strCount
